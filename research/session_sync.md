...
- Follow-up P5 probes showed the vision-tower failure is now resolved by text-only Qwen3.5 vision isolation, but a new real failure appears in a trainable language flat parameter after the first SDPO actor update. Codex added optimizer-step forensics behind `SDPO_FAIL_FAST_NONFINITE=1`: actor params before backward, scalar loss before backward, gradients before optimizer step, actor params before optimizer step, optimizer state before/after optimizer step, and actor params after optimizer step. The next Kiro/P5 run should classify this as pre-step param, loss/backward, optimizer-state, post-step update, or EMA propagation rather than guessing "extreme JSD gradient" upfront.

...
- Ref/EMA teacher micro-batch config is now defined in `ref.yaml` via `actor_rollout_ref.ref.ppo_micro_batch_size_per_gpu`, and the worker preserves that value when `log_prob_micro_batch_size_per_gpu` is unset. This fixes the `ActorConfig` construction path where ref log-prob defaults could otherwise overwrite the ppo micro-batch field with `None`.

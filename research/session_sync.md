...
- New faithful launch contract: `SDPO_ARM=peer_only`, `TAU3_LIVE_FEEDBACK_FORMAT=none`, `TAU3_LIVE_RUNTIME=official_gym`, `TAU3_MARK_ENV_EXCEPTIONS=false`, `tau3.sdpo.use_successful_peer_solution=true`, `tau3.sdpo.only_failed_rollouts=true`, `tau3.sdpo.target_guard.enabled=false`, and `tau3.sdpo.memory` absent/disabled.
- The P5 launcher now rejects proxy/legacy runtimes, heuristic feedback, Note-SDPO memory, custom teacher templates, and target-guard enabling for claim-bearing faithful runs.
- The faithful recipe defaults are `TRAIN_BATCH_SIZE=4`, `PPO_MINI_BATCH_SIZE=4`, `ROLLOUT_BATCH_SIZE=8`, `VAL_N=4`, `MAX_PROMPT_LENGTH=8192`, `MAX_RESPONSE_LENGTH=6144`, `MAX_MODEL_LEN=14336`, `SDPO_MAX_REPROMPT_LEN=6144`, `TOTAL_TRAINING_STEPS=240`, `TOTAL_EPOCHS=30`, `TEST_FREQ=30`, and `SAVE_FREQ=30`.
- No-selected peer-only SDPO batches now skip actor/critic update and EMA teacher update, so all-fail/no-peer groups cannot move the actor through zero-loss AdamW weight decay.
...
- Follow-up P5 probes showed the vision-tower failure is now resolved by text-only Qwen3.5 vision isolation, but a new real failure appears in a trainable language flat parameter after the first SDPO actor update. Codex added optimizer-step forensics behind `SDPO_FAIL_FAST_NONFINITE=1`: actor params before backward, scalar loss before backward, gradients before optimizer step, actor params before optimizer step, optimizer state before/after optimizer step, and actor params after optimizer step. Probe v5 ruled out cast overflow: the source actor value was small/finite fp32 on CUDA, target was fp32 CPU, and only the CUDA-to-CPU EMA transfer produced the non-finite. Current patch makes the EMA actor-to-teacher transfer blocking (`non_blocking=False`) and synchronizes before fail-fast finite validation.
...
- Added `scripts/p5_run_east_original_sdpo_full.sh` for east-P5 SDPO experiments. For claim-bearing faithful SDPO, Recipe 11 should call `run_local_tau3_sdpo_live_p5.sh` directly with the 6K/14,336-token caps; capacity-matrix helpers are not the canonical faithful run path.
...
- Target-side SDPO guardrails remain available as an explicit ablation via `tau3.sdpo.target_guard`, but the faithful Tau3 SDPO baseline defaults them off. Successful peer demonstrations are selected by same-UID reward success only, then stripped of `<think>` before teacher reprompting.
- The actor loss still consumes an explicit `self_distillation_target_token_mask`; in the faithful default this is the assistant response mask gated only by peer-only row selection.
...
- Claim boundary: this is the faithful original-style Tau3 SDPO baseline under the default launch shape: successful peer only, no diagnostic feedback, no memory, target guard disabled, with runtime/tensor safety checks retained. Target guard, feedback, and memory are separate ablations.

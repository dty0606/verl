...
- Ref/EMA teacher micro-batch config must be populated through `actor_rollout_ref.ref.log_prob_micro_batch_size_per_gpu`; the worker also preserves an explicitly supplied `ref.ppo_micro_batch_size_per_gpu` as fallback. This fixes the `ActorConfig` construction path where ref log-prob defaults could otherwise overwrite the ppo micro-batch field with `None`.

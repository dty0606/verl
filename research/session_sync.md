...
- Follow-up P5 probes showed the vision-tower failure is now resolved by text-only Qwen3.5 vision isolation, but a new real failure appears in a trainable language flat parameter after the first SDPO actor update. Codex added optimizer-step forensics behind `SDPO_FAIL_FAST_NONFINITE=1`: actor params before backward, scalar loss before backward, gradients before optimizer step, actor params before optimizer step, optimizer state before/after optimizer step, and actor params after optimizer step. Probe v4 still first caught the language issue inside EMA, but specifically after actor-to-teacher device/dtype transfer. The next patch records the original actor source value/dtype/device and teacher target dtype/device at the first bad transferred index, so Kiro/P5 can distinguish cast overflow from a genuinely non-finite actor tensor.
